        ...
        <header className="sticky top-0 z-50 w-full border-b border-black/5 dark:border-white/5 bg-white/30 dark:bg-black/30 backdrop-blur-xl">
            <div className="container mx-auto flex items-center justify-between p-4">
                ...
                <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-stone-600 dark:text-stone-400">
                    {navLinks.map((link) => (
                        <Link key={link.href} href={link.href} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
                            {link.label}
                        </Link>
                    ))}
                </nav>
                ...
                <div className="flex items-center gap-2">
                    <Button variant="ghost" className="hidden sm:inline-flex text-stone-600 dark:text-stone-400 hover:bg-stone-200 dark:hover:bg-neutral-800" asChild>
            ...
            {isMenuOpen && (
                <div className="md:hidden bg-white/90 dark:bg-black/90 backdrop-blur-lg border-t border-black/5 dark:border-white/5">
                    <nav className="container mx-auto flex flex-col p-4 gap-2">
                        {navLinks.map((link) => (
                            <Link key={link.href} href={link.href} className="py-2 text-center text-stone-700 dark:text-stone-300 rounded-md hover:bg-stone-100 dark:hover:bg-neutral-800" onClick={toggleMenu}>
                                {link.label}
                            </Link>
                        ))}
